...
public:
    ...
    transformer_encoder_layer(register_module("encoder_layer", 
        torch::nn::TransformerEncoderLayer(
            torch::nn::TransformerEncoderLayerOptions(HIDDEN_SIZE, NUM_HEADS).dropout(0.1)))),
    transformer_encoder(register_module("encoder", 
        torch::nn::TransformerEncoder(transformer_encoder_layer, NUM_LAYERS))),
    fc_out(register_module("fc_out", torch::nn::Linear(HIDDEN_SIZE, 1))),  // Single output neuron
    positional_encoding(register_module("positional_encoding", 
        std::make_shared<PositionalEncoding>(HIDDEN_SIZE, SEQ_LEN))),
    ...
    torch::Tensor forward(torch::Tensor input) {
        ...
        if (forward_count % DEBUG_INTERVAL == 0) {
            std::cout << "Forward pass: Input shape: " << input.sizes() << std::endl;
        }
        ...
        if (input.size(1) > SEQ_LEN) {
            input = input.index({torch::indexing::Slice(), torch::indexing::Slice(0, SEQ_LEN), torch::indexing::Slice()});
        }
        
        input = input.transpose(0, 1); // (SEQ_LEN, batch_size, HIDDEN_SIZE)
        auto memory = transformer_encoder(input);
        memory = memory.transpose(0, 1); // (batch_size, SEQ_LEN, HIDDEN_SIZE)
        
        if (forward_count % DEBUG_INTERVAL == 0) {
            std::cout << "Memory shape after transformer: " << memory.sizes() << std::endl;
        }
        
        auto memory_pooled = memory.mean(1); // Mean pooling over timesteps
        if (forward_count % DEBUG_INTERVAL == 0) {
            std::cout << "Memory pooled shape: " << memory_pooled.sizes() << std::endl;
        }
        
        auto output = torch::sigmoid(fc_out(memory_pooled));
        if (forward_count % DEBUG_INTERVAL == 0) {
            std::cout << "Output shape: " << output.sizes() << " Value: " << output << std::endl;
        }
...
torch::Tensor TransformerPredictor::predict(uint64_t ip) {
    ...
    if (forward_count % DEBUG_INTERVAL == 0) {
      //  std::cout << "Predict: Input tensor shape: " << input.sizes() << " Values: " << input << std::endl;
    }
...
void TransformerPredictor::update(uint64_t ip, uint8_t taken) {
    ...
    //std::cout << "Update: Prediction: " << prediction << " Target: " << target << " Loss: " << loss << std::endl;
    ...
    if (TOGGLE_LEARNING) {
        ...
        loss.backward();
        this->optimizer->step();
     //   std::cout << "Gradient update complete" << std::endl;
    }
}
...
uint8_t O3_CPU::predict_branch(uint64_t ip) {  
    return transformer_net[cpu].predict(ip).item<float>() > 0.5 ? 1 : 0;  // Convert sigmoid output to binary decision
}
